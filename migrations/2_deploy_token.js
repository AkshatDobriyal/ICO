...
module.exports = function (deployer, network, accounts) {
    ...
    const _rate = 4761904; // token price = $0.001 = 2.1e-7 ETH => rate = 1 / 2.1e-7 = 4761904.76
    const _wallet = accounts[0];
    const _token = deployedToken.address;
    const _cap = ether(2669); // cap = USD 12.5 million = 2669.14 ETH
    const _openingTime = latestTime + duration.days(1); 
    const _closingTime = _openingTime + duration.weeks(4); // duration = 30 days = 4 weeks
    const _goal = ether(1068); // soft cap = $5,000,000 = 1067.98 ETH
